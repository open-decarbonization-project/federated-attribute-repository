...
/**
 * Recursive-descent parser for OData-style filter expressions. Supports {@code eq, ne, gt, ge, lt,
 * le, in, contains()} operators with {@code and}/{@code or} combinators and parenthesized grouping.
 * Also provides utilities to extract namespace predicates and strip them for federated query
 * delegation.
 */
  ...
  private static Filter remove(final Filter filter) {
    switch (filter) {
      case Filter.Comparison comparison -> {
        if ("namespace".equals(comparison.field())
            && (comparison.operator() == Operator.EQ || comparison.operator() == Operator.IN)) {
          return null;
        }
        return comparison;
      }
